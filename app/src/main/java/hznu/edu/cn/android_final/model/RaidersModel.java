...
public class RaidersModel {
    // 调用servlet验证登录
    public static void sendRequestWithOkHttp_for_raiders(final String search_type) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    RequestBody requestBody=new FormBody.Builder().add("type",search_type).build();
                    Request request1=new Request.Builder().url("http://10.0.2.2:8888/JSP_Final/Leaderboard_Servlet").post(requestBody).build();
                    ...
                    msg.what=2;
                    msg.setData(bundle);
                    RaidersFragment.mHandler.sendMessage(msg);//向Handler发送消息，
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
